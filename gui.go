...
func InstallLatestBuilds() (err error) {
	...
	if err != nil {
		ShowModal("Uh Oh!", "Failed to install the latest Lofycord builds from GitHub:\n"+err.Error())
	}
	return
}
...
func renderInstaller() g.Widget {
	...
	layout := g.Layout{
		...
		g.Style().SetFontSize(20).To(
			renderErrorCard(
				DiscordYellow,
				"**Github** is the only official place to get Lofycord. Any other site claiming to be us is malicious.\n"+
				...
				g.Style().
					...
					To(
						g.Button("Reinstall / Repair").
							...
							Size((w-40)/4, 50),
						Tooltip("Reinstall & Update Lofycord"),
					),
				g.Style().
		...
		InfoModal("#patched", "Successfully Patched", "If Discord is still open, fully close it first.\n"+
			"Then, start it and verify Lofycord installed successfully by looking for its category in Discord Settings"),
		...
		InfoModal("#scuffed-install", "Hold On!", "You have a broken Discord Install.\n"+
			...
			"You need to fix this before patching, otherwise Lofycord will likely not work.\n\n"+
		...
		RawInfoModal("#openasar-confirm", "OpenAsar", "OpenAsar is an open-source alternative of Discord desktop's app.asar.\n"+
			"Lofycord is in no way affiliated with OpenAsar.\n"+
...
func loop() {
	...
	g.SingleWindow().
		...
		Layout(
			...
			g.Style().SetFontSize(20).To(
				g.Row(
					g.Label(Ternary(IsDevInstall, "Dev Install: ", "Lofycord will be downloaded to: ")+LofycordDirectory),
					g.Style().
						SetColor(g.StyleColorButton, DiscordBlue).
						SetStyle(g.StyleVarFramePadding, 4, 4).
						To(
							g.Button("Open Directory").OnClick(func() {
								g.OpenURL("file://" + path.Dir(LofycordDirectory))
							}),
						),
				),
				&CondWidget{!IsDevInstall, func() g.Widget {
					return g.Label("To customise this location, set the environment variable 'Lofycord_USER_DATA_DIR' and restart me").Wrapped(true)
				}, nil},
				g.Dummy(0, 10),
				g.Label("Equilotl Version: "+buildinfo.InstallerTag+" ("+buildinfo.InstallerGitHash+")"+Ternary(IsSelfOutdated, " - OUTDATED", "")),
				g.Label("Local Lofycord Version: "+InstalledHash),
				&CondWidget{
					GithubError == nil,
					func() g.Widget {
						if IsDevInstall {
							return g.Label("Not updating Lofycord due to being in DevMode")
						}
						return g.Label("Latest Lofycord Version: " + LatestHash)
					}, func() g.Widget {
