...
func InitGithubDownloader() {
	...
	IsDevInstall = os.Getenv("LOFYCORD_DEV_INSTALL") == "1"
	...
	LofycordFile := LofycordDirectory

	stat, err := os.Stat(LofycordFile)
	if err != nil {
		return
	}
	...
	// dev
	if stat.IsDir() {
		LofycordFile = path.Join(LofycordFile, "main.js")
	}

	// Check hash of installed version if exists
	b, err := os.ReadFile(LofycordFile)
	if err != nil {
		return
	}

	Log.Debug("Found existing Lofycord Install. Checking for hash...")

	re := regexp.MustCompile(`// Lofycord (\w+)`)
...
func installLatestBuilds() (retErr error) {
	...
	out, err := os.OpenFile(LofycordDirectory, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
		Log.Error("Failed to create", LofycordDirectory+":", err)
		retErr = err
		return
	}
	read, err := io.Copy(out, res.Body)
	if err != nil {
		Log.Error("Failed to download to", LofycordDirectory+":", err)
		retErr = err
		return
	}
	...
	_ = FixOwnership(LofycordDirectory)
